...
signal second_touch_pressed
signal second_touch_released

var _second_touch_index : int = -1
...
func _input(event: InputEvent) -> void:
	if event is InputEventScreenTouch:
		if event.pressed:
			...
			elif _touch_index != -1 and _second_touch_index == -1:
				_second_touch_index = event.index
				emit_signal("second_touch_pressed")
				get_viewport().set_input_as_handled()
			...
			emit_signal("second_touch_released")
			get_viewport().set_input_as_handled()
